                ...
                <table class="custom-table">
                    ...
                    <tbody id="eventsTableBody">
                        ...
                        <tr class="event-row" data-name="{{ strtolower($event->title) }}" data-location="{{ strtolower($event->location) }}">
                            ...
                            <td>@if($event->price > 0)<span class="badge paid">${{ number_format($event->price, 2) }}</span>@else<span class="badge free">Free</span>@endif</div>
                        ...
                        @empty
                        <tr><td colspan="7"><div class="empty-state"><i class="fas fa-calendar-times" style="font-size:40px;"></i><p>No events found.</p></div></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    // Get replied message IDs from localStorage
    ...
    async function loadAllMessages() {
        ...
        try {
            ...
            // Filter: Show ONLY user messages (not admin replies)
    ...
    document.getElementById('eventsTableBody')?.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.view-btn')) { 
            ...
            // Set the event name in modal
            document.getElementById('attendeesEventName').textContent = eventTitle; 
            document.getElementById('attendeesList').innerHTML = '<div class="text-center py-4"><div class="spinner-border text-primary"></div><p>Loading attendees...</p></div>'; 
            ...
            .then(data => { 
                ...
                const attendees = Array.isArray(data) ? data : (data?.data || []); 
                const list = document.getElementById('attendeesList'); 
                if (!attendees.length) { 
                    list.innerHTML = '<div class="text-center py-4"><i class="fas fa-user-slash fa-3x mb-2" style="opacity:0.5;"></i><p>No registered users for this event.</p></div>'; 
                    return; 
                } 
                list.innerHTML = attendees.map(user => `
                    <div class="d-flex justify-content-between align-items-center py-3" style="border-bottom:1px solid rgba(255,255,255,0.1);">
                        <div>
                            <strong>${escapeHtml(user.name)}</strong>
                            <br>
                            <small class="text-muted">${escapeHtml(user.email || '')}</small>
                        </div>
                        <small class="text-muted">${user.pivot?.created_at ? new Date(user.pivot.created_at).toLocaleDateString() : (user.registered_at ? new Date(user.registered_at).toLocaleDateString() : 'Registered')}</small>
                    </div>
                `).join(''); 
            })
            .catch(error => { 
                console.error('Error loading attendees:', error);
                document.getElementById('attendeesList').innerHTML = '<div class="text-center py-4 text-danger"><i class="fas fa-exclamation-triangle fa-2x mb-2"></i><p>Failed to load attendees. Please try again.</p><small>Error: ' + error.message + '</small></div>'; 
            }); 
        }
